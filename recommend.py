...
def find_centroid(cluster):
    ...
    lat,lon = [],[]
    for c in cluster:
        lat += [restaurant_location(c)[0]]
        lon += [restaurant_location(c)[1]]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    #the user's ratings for the restaurants in restaurants
    ys = [reviews_by_user[restaurant_name(r)] for r in restaurants]

    # BEGIN Question 7
    "*** REPLACE THIS LINE ***"
    res_with_rate = zip(xs,ys)
    sxx,syy,sxy = 0,0,0
    for x,y in res_with_rate:
        sxx += (x-mean(xs))**2
        syy += (y-mean(ys))**2
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for p in pairs:
        if p[1]>biggest[1]:
            biggest = p
    return find_predictor(user,reviewed,biggest[0])[0]
...
def rate_all(user, restaurants, feature_fns):
    ...
    #list of restaurants reviewed by the user
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
